...
def test_summarise_precision_recall() -> None:
    ...
    con.sql(
        ...
        FROM (
            VALUES
                ...
                ('a3', '12', NULL, NULL),
                ('a4', '13', '13', 'NGRAM')
        ) AS t(unique_id, ukam_label, resolved_canonical_id, match_reason)
        """
    )

    summary = summarise_precision_recall(con, "matches").fetchone()

    assert summary[0] == 4
    assert summary[1] == 3
    assert summary[2] == 2
    assert summary[3] == pytest.approx(0.666667, rel=1e-6)
    assert summary[4] == pytest.approx(0.5, rel=1e-6)


def test_summarise_by_match_reason() -> None:
    con = duckdb.connect(database=":memory:")
    con.sql(
        """
        CREATE TABLE matches AS
        ...
        FROM (
            VALUES
                ('a1', '10', '10', 'exact: full match'),
                ('a2', '11', '99', 'exact: full match'),
                ('a3', '12', NULL, NULL),
                ('a4', '13', '13', 'ngram_jaccard: fuzzy ngram match')
        ) AS t(unique_id, ukam_label, resolved_canonical_id, match_reason)
        """
    )

    match_metrics = con.sql(
        """
        SELECT
            match_reason,
            ...
            printf('%.2f%%', 100.0 * COUNT(*) / SUM(COUNT(*)) OVER ())
                AS match_percentage
        FROM (
            SELECT COALESCE(match_reason, 'unmatched') AS match_reason
            FROM matches
        )
        GROUP BY match_reason
        ORDER BY match_count DESC, match_reason
        """
    )

    rows = summarise_by_match_reason(con, "matches", match_metrics).fetchall()
    ...
    assert rows == [
        ...
        (
            "ngram_jaccard: fuzzy ngram match",
            4,
            1,
...
def test_summarise_run_totals() -> None:
    con = duckdb.connect(database=":memory:")
    con.sql(
        """
        CREATE TABLE matches AS
